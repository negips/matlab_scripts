...
% Forcing field for Optimal forcing
forc_fld = rand([length(xall),1]);
fopt = gs_scatter(forc_fld,N,nels,periodic);
fopt = fopt/norm(fopt,2);


% Initialization of lag arrays
ulag1 = 0*un;
ulag2 = 0*un;
...
opt_forcing_lag1 = 0*un;
opt_forcing_lag2 = 0*un;


nek_dir_stiff = -nek_conv - nek_lp;
nek_adj_stiff =  nek_conv - nek_lp;
%nek_bc = nek_cbc;
mass = 0*nek_mass;
stiff = 0*nek_dir_stiff;
...
nsteps=100000;
surfupd = 5*isave;               % make surface evry surfupd of a period
verbose=0;
verbosestep=500;
ifpcg=0;
ifdealias=0;
...
for loop=1:nloops
  ...
  fopt = un;            % use the output as the forcing field

  figure(2)
  plot(xgll(:),real(fopt(:)), '-b'); hold on
  plot(xgll(:),imag(fopt(:)), '--r'); hold on
  plot(xgll(:),abs(fopt(:)), '--k'); hold off
  ...
  for i = 1:nsteps
    ...
    for els=1:nels
      ...
      if (~ifdir) && els==nels
%       Add adjoint boundary condition for Outflow 
        mass(end,end,els) = mass(end,end,els)-nu;
      end        
...
%     Conv + dissipation
      if (ifdir)      
        stiff(:,:,els) = nek_dir_stiff(:,:,els);
      else
        stiff(:,:,els) = nek_adj_stiff(:,:,els);
      end        
  
      b21 = extk(2)*un(:,els);
      b22 = extk(3)*ulag1(:,els);
      b23 = extk(4)*ulag2(:,els);
      b_stiff = deltat*nek_mass(:,:,els)*stiff(:,:,els)*(b21 + b22 + b23);
...
%     Optimal Forcing term
      opt_forcing_term(:,els) = nek_mass(:,:,els)*fopt(:,els)*exp(io*OMEGA*time);
  
      b_optforcing1 = extk(2)*opt_forcing_term(:,els);
      b_optforcing2 = extk(3)*opt_forcing_lag1(:,els);
      b_optforcing3 = extk(4)*opt_forcing_lag2(:,els);
      b_optforcing  = deltat*(b_optforcing1 + b_optforcing2 + b_optforcing3);
  
%     sum for rhs
      b(:,els) = -b_bdf + b_stiff + b_source + b_cubic + b_quintic + b_forcing + b_optforcing;
...
%%  Lag terms  
    ...
    opt_forcing_lag2 = opt_forcing_lag1;
    opt_forcing_lag1 = opt_forcing_term;
%%
