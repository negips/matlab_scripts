...
%		err	estimated absolute error on eigenvalues
...
if bc=='N'
  [eigvec,eigval,x,err]=sem1d_modal_analysis(xmesh,rho,mu,ngll,'NN');
  k=(0:length(eigval)-1)';
else
  [eigvec,eigval,x,err]=sem1d_modal_analysis(xmesh,rho,mu,ngll,'DD');
  k=(1:length(eigval))';
end
anaval=pi*k;
...
%------
function prop=constant(data,e,x)
prop=repmat(data,length(x),1);
