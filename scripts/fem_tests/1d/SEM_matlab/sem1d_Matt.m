...
nglob = NEL*(NGLL-1) + 2;	% number of global nodes
...
CFL = 0.8; 			% stability number
	% NOTE: the critical CFL is smaller
	%	than in the case of no linear-slip-fault (0.85)
...
for e=1:NEL, % FOR EACH ELEMENT ...
 ...
 % from local data (contributions from each element)
  ...
  if e>NEL/2, iglob(:,e) = iglob(:,e)+1; end
...
% linear slip fault with split nodes
