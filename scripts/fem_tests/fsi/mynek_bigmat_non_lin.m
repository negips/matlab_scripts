...
%nekinit5
%load 'matrices_N8_NELV4_def2'
load 'matrices_N11_NELV4_CART5_1'
...
%cx = ones(size(velvec));
%cy = zeros(size(velvec));


hini=figure;
ifsolnplot=1;
El = UpdSoln(El,velvec,gno,nelv,lx1,ly1,hini,ifsolnplot);

%% Convecting velocity
for elno=1:nelv
     El(elno).cx = 2 + El(elno).un;
     El(elno).cy = zeros(size(El(elno).un));
end
...
%    c_op = bigconvd*un;
     ifnek=0;     
     [c_op] = convop(Nx,Ny,Nxd,Nyd,El,nelv,nbasis,ifnek);
     b21 = extk(2)*c_op;
     b22 = extk(3)*blag1;
     b23 = extk(4)*blag2;
     b2 = (b21 + b22 + b23);
