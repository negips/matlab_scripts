...
% Check Divergence
[divergence dealiased_divergence] = GetDivNorm(El,nelv);
dnorm = divergence/volume;
ddnorm = dealiased_divergence/volume;

disp(['Divergence Norm: ' num2str(dnorm)])
disp(['Dealiased Divergence Norm: ' num2str(ddnorm)])
...
%%   Visualize Convecting field in y 
    ...
    figure(105)
    scatter(El(ii).xm1(:),El(ii).ym1(:), '.k'); hold on
    title('GLL points')
