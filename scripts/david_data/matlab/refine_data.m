...
%%
h4=figure;
scatter(aoa0r1,k2ampr1./k1ampr1,[],reccoder1);
ylabel('k2_{amp}/k1_{amp}', 'Interpreter', 'tex', 'FontSize', fs)
xlabel('\alpha', 'Interpreter', 'tex', 'FontSize', fs)
xlim([-2 15])
filename='alpha_k2amp_r.eps'
filename = [num2str(deltar1(1)) '_' filename];
...
while button==1
  ...
  if ifalphaplot
    dist = sqrt((amp_ratio-yp).^2 +(aoa0r1-xp).^2);
  else
...
if ifpressure
  ...
  xt=segments(iseg).xtaps(toptaps);
  yt=segments(iseg).ytaps(toptaps);
  itaps=segments(iseg).itaps;

  time=segments(iseg).ptime;  
  pressure = segments(iseg).pressure(:,itaps);
  ptop = pressure(:,toptaps);
  ...
  for ip = 1:nlocs
    xl = locations(ip);
    [val ind9] = min(abs(xt-xl));
    x_ind = [x_ind ind9];
    leg2{ip} = ['x=' num2str(xl)];
  end

  p_x = transpose(ptop(:,x_ind));
  plot(time,p_x)
  legend(leg2,'Interpreter','tex','FontSize', fs)
