...
while button==1
  ...
  plot(q_alpha*180/pi,shifted_q_cz, 'Color', col1(1,:))
...
%  [pxx,f2] = pwelch(zero_mean_cz,window,n_overlap,nfft,p_fs);
  ...
  psd_all = plot(k2(ind4),pxx_shifted, 'Color', col1(1,:)); hold on
  pxx_new = pxx(ind4);
  pxx_max = max(pxx_new);

  ind5 = find(pxx_new>tol_secondary*pxx_max);
  pxx_new(ind5) = 0.;
  pxx_new_norm = pxx_new/(max(pxx_new)) +iseg-1;
  psd_fil = plot(k2(ind4),pxx_new_norm, '--', 'Color', col1(1,:)); hold on
