...
if y1plus>firstpt           % we want first point at y+<0.90
     dr_min    = min(diff(dr));
     dr_min_el = firstpt/Re_tau;
     el_min    = 2*dr_min_el/dr_min;         % max element size at the wall
                                             % This is the min el size in the mesh
end
...
ex_r = 1.050;         % grid expansion ratio

ny_pts = floor(1 + log10(el_max/el_min)/log10(ex_r));
Nel = ny_pts;
% check lenth of ny_pts
sn = el_min*(1-ex_r^(ny_pts-1))/(1-ex_r);

if sn<=1             % Good. Constant spacing in the rest of the region.
...
else                 % Spacing doesn't reach constant value

     ny_pts = 1 + log((el_min + (ex_r-1)*1)/el_min)/log(ex_r);
     if (ny_pts - floor(ny_pts) > 0)
          ny_pts = ceil(ny_pts);
     end
     ...
     if isreal(ex_r) ~= 1 || ex_r <= 1
          display('Error in Polynomial root finding')
          break
     end
 
     sn = el_min*(1-ex_r^(ny_pts))/(1-ex_r);
     Nel = ny_pts;
     length_equi=1-sn;
     twice_len = length_equi*2;
     nel_equi = twice_len/el_max;
...
%Combine Geometric and uniform distribution
nodes(1) = 1;
for i = 1:Nel
     el_s = el_min*ex_r*(i-1);
     els_len = el_min*(1-ex_r^i)/(1-ex_r);
     nodes(i+1) = 1 - els_len;
end

if needed_equi > 0 
for i = 1:needed_equi
     equinodes(i) = 1 - els_len - i*el_max;
end
end
%break
...
l1=length(allnodes);
for i = 1:needed_equi
     allnodes(l1+i) = allnodes(l1+i-1)-el_max;
end
