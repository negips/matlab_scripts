function [A,B] = SuctionMatrix(N,alpha,beta,Re)
...
%    yinf = 3;
    ...
    V0=-1/Re    % ==> displacement thickness normalization 
    ...
    y0  = min(yphys);                       % y0 
    u   = ones(length(vec),1) - exp(V0*Re*(yphys-y0));         % U velocity profile (physical)
    v   = V0*ones(length(vec),1);           % V velocity profile (Physical)       
    du  = -V0*Re*exp(V0*Re*(yphys-y0));     % y derivative (physical)
    ddu = V0*Re*du;                         % 2nd derivative    
