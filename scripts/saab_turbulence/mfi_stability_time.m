...
sdata = importdata('surf_normals.34.N9');
itop = find(sdata.data(:,5)>=0);
ibot = find(sdata.data(:,5)<0);
sdata_xt = sdata.data(itop,1);
sdata_yt = sdata.data(itop,2);
sdata_snxt = sdata.data(itop,4);
sdata_snyt = sdata.data(itop,5);

stab_x0 = 0.10;

[val ind] = min(abs(sdata_xt-stab_x0));
stab_x0 = sdata_xt(ind);
stab_y0 = sdata_yt(ind);
...
%   Plot a profile
    ...
    Omg = 2*pi/T;
    T0=6.0;
    dalpha = 1.0*pi/180*sin(Omg*(les.timee-T0)-pi/2);

%   Clockwise rotation    
    R = [cos(dalpha)  sin(dalpha); ...
         -sin(dalpha) cos(dalpha)];

    Coords = R*[stab_x0-0.35; stab_y0-0.034];
    stab_x = Coords(1)+0.35;
    ...
    legx{cnt}=['$x=',num2str(lesdata(ind).xa,4), '$'];

    figure(4)
    plot(u_prof,yn_prof,'Color', cols_mfi(cnt,:)); hold on
    xlabel('$U$')
    ylabel('$y_{n}$')
    ylim([0 0.1])
  ...
  %      break
    ...
    if length(uns_ar)>0 
      figure(301)
      plot(uns_ar,uns_wi, '. ', 'Color', cols_mfi(cnt,:)); hold on
      ylabel('$\lambda_{i}$')
      xlabel('$\alpha_{r}$')
      grid on
      legend(legx,'Interpreter','latex', 'FontSize',lgfs, 'Location', 'best')     
      ...
      figure(7)
      plot(uns_wr,uns_wi, '. ', 'Color', cols_mfi(cnt,:)); hold on
