...
svfname = 'wing_ff2w3_2d_p8_pitch.mat';
...
for tc=1:topcount
  j=tindicies(tc);
  Ralphap=[cos(top(tc).alpha) sin(top(tc).alpha) 0; ...
      -sin(top(tc).alpha) cos(top(tc).alpha) 0; ...
      0 0 1];
  for i=1:ln
...
for bc=1:botcount
  j=bindicies(bc);
  Ralphapp=[cos(bottom(bc).alpha) sin(bottom(bc).alpha) 0; ...
      sin(bottom(bc).alpha) -cos(bottom(bc).alpha) 0; ...
      0 0 1];
  for i=1:ln
...
clearvars -except top bottom svfname fname Rer Domain mnelxyz nel Poly nstat nderiv times timee atime DT nrec Tint npoints wallpts nypts streamwise
