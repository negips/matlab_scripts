...
%Geometry of NACA profile
%Geometry of symmetric NACA
c=1; %Chord length
T=12; %Percentage of thickness to chord (two last digits)
t=T/100; %Maximum thickness as a function of chord

%Geometry of cambered NACA
M=4; %Percentage of maximum camber (first digit)
Pc=4; %Location of maximum camber (second digit)
m=4/100; %Maximum camber as a function of chord
p=Pc/10; %Location of maximum camber as a function of chord

%We take the friction velocity at x/c around 0.65, which is the point where
%the skin friction from top and bottom surfaces is roughly the same. After
%this point, the top surface starts to separate.
Rec=400e3;
if Rec==400e3
    utz=0.0471;
end

%Thicknesses of wall-normal profile delta1
delta1=0.2;

%After 30% chord distance we will design a uniform spacing in the
%interpolating mesh, with 40 points
dx=0.7/40;
...
%uniform value dx.
%We use a total of 40 points in this region.
nle=40; 
...
%ratio
x1=0; %Location of first point (leading edge)
xn=0.3; %Location of last point (after which uniform dx is considered)
